...
class ExportPolicy
{
    ...
    */
    ...
    {
        return app(VisibilityScopeResolver::class)->canAccess(
            $user,
            $model,
            'view-all-export-file',
            'view-own-export-file',
            fn (Authenticatable $user, ExportFile $model) => $this->userOwnsExportFile($user, $model)
        );
    }
    ...
    private function userOwnsExportFile(Authenticatable $user, ExportFile $model): bool
    {
        ...
        return method_exists($user, 'locationIds')
            && $user->locationIds()->contains($model->location_id);
    }
}
